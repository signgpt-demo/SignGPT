...
class BaseModel(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Show live loss on the progress bar and TensorBoard at every step.
        # We also pull text_loss / motion_loss off the LM loss wrapper when
        # available so the progress bar shows the split.
        if loss is not None:
            try:
                self.log("train/loss_step",
                         loss,
                         on_step=True,
                         on_epoch=False,
                         prog_bar=True,
                         logger=True,
                         sync_dist=True)
                # Locate the wrapped loss output produced by Base_MLM /
                # MLM so we can split text vs motion on the progress bar.
                wrapped = None
                lm = getattr(self, "lm", None)
                if lm is not None:
                    wrapped = getattr(lm, "last_loss_output", None)
                if wrapped is not None:
                    if getattr(wrapped, "text_loss", None) is not None:
                        self.log("train/text_loss_step",
                                 wrapped.text_loss.detach(),
                                 on_step=True,
                                 on_epoch=False,
                                 prog_bar=True,
                                 logger=True,
                                 sync_dist=True)
                    if getattr(wrapped, "motion_loss", None) is not None:
                        self.log("train/motion_loss_step",
                                 wrapped.motion_loss.detach(),
                                 on_step=True,
                                 on_epoch=False,
                                 prog_bar=True,
                                 logger=True,
                ...
                # Logging is best-effort — never break training because of it.
                pass
        return loss

    def on_train_batch_end(self, *args, **kwargs):
        # Per-step console printing is disabled — the rich progress bar
        # already shows train/loss_step, train/text_loss_step and
        # train/motion_loss_step live. To re-enable, set
        # ``self._loss_print_every_n`` to a positive integer.
        return
    ...
    def on_train_epoch_end(self):
        ...
        # Log losses
        loss_dico = self.loss_log_dict('train')
        dico.update(loss_dico)
        ...
        if not self.trainer.sanity_checking:
            ...
            if loss_dico:
                ...
                parts = [f"{'_'.join(k.split('/')[:-1])}: {v:.4e}" if isinstance(v, float) and v != 0.0 else None for k, v in loss_dico.items()]
    ...
    def on_validation_epoch_end(self):
        ...
        metrics_dico = self.metrics_log_dict()
        dico.update(metrics_dico)

        metrics_dico_test = self.metrics_log_dict(use_test_metrics=True)
        dico.update(metrics_dico_test)

        if not self.trainer.sanity_checking:
            self.log_dict(dico, sync_dist=True, rank_zero_only=True)
            epoch = self.trainer.current_epoch
            if metrics_dico:
                parts = [f"{k}: {v:.4f}" if isinstance(v, float) and v != 0.0 else None for k, v in metrics_dico.items()]
                parts = [p for p in parts if p is not None]
                if parts:
                    print(f"\n[Epoch {epoch}] Val(train_part): {' | '.join(parts)}")
            if metrics_dico_test:
                parts = [f"{k}: {v:.4f}" if isinstance(v, float) and v != 0.0 else None for k, v in metrics_dico_test.items()]
                parts = [p for p in parts if p is not None]
                if parts:
                    print(f"[Epoch {epoch}] Val(test):       {' | '.join(parts)}\n")
    ...
    def preprocess_state_dict(self, state_dict):
        ...
        metrics_test_state_dict = self.metrics_test.state_dict()

        for k, v in metric_state_dict.items():
            new_state_dict['metrics.' + k] = v

        for k, v in metrics_test_state_dict.items():
            new_state_dict['metrics_test.' + k] = v

        for k, v in loss_state_dict.items():
            new_state_dict['_losses.' + k] = v

        for k, v in state_dict.items():
            if '_losses' not in k and 'Metrics' not in k and 'metrics_test' not in k:
                new_state_dict[k] = v
    ...
    def metrics_log_dict(self, use_test_metrics=False):
        if self.trainer.datamodule.is_mm and "TM2TMetrics" in self.hparams.metrics_dict:
            metrics_dicts = ['MMMetrics']
        else:
            metrics_dicts = self.hparams.metrics_dict

        metrics_obj = self.metrics_test if use_test_metrics else self.metrics
        prefix = "TestMetrics" if use_test_metrics else "Metrics"
        ...
        for metric in metrics_dicts:
            ...
            metrics_log_dict.update({
                f"{prefix}/{metric_name}": value.item()
    ...
    def configure_metrics(self):
        ...
        self.metrics_test = BaseMetrics(datamodule=self.datamodule, **self.hparams)
