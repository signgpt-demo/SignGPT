...
def _resolve_env_with_default(spec):
    """Resolve `${VAR:-default}` style env-var interpolation.

    Falls back to the literal default when the env var is unset or empty.
    """
...
def parse_args(phase="train"):
    ...
    # Assets: default to ./configs/assets.yaml relative to repo root (override via --cfg_assets or env)
    _repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _default_assets = os.path.join(_repo_root, "configs", "assets.yaml")
    ...
    group.add_argument(
        ...
        default=os.environ.get(
            "SIGNGPT_CFG_ASSETS", os.environ.get("MGPT_CFG_ASSETS", _default_assets)
        ),
        help="config file for asset paths (default: configs/assets.yaml)",
    )

    # Default experiment config per phase
    _configs_dir = os.path.join(_repo_root, "configs")
    if phase in ["train", "test", "demo"]:
        cfg_defualt = os.path.join(_configs_dir, "config_h3d_stage1.yaml")
    elif phase == "render":
        cfg_defualt = os.path.join(_configs_dir, "render.yaml")
    elif phase == "webui":
        cfg_defualt = os.path.join(_configs_dir, "webui.yaml")
    else:
        cfg_defualt = os.path.join(_configs_dir, "config_h3d_stage1.yaml")
    ...
    group.add_argument(
        ...
        type=str,
        required=False,
        default=os.environ.get("SIGNGPT_CFG", os.environ.get("MGPT_CFG", cfg_defualt)),
        help="config file",
    )
    ...
    if phase == "demo":
        ...
        group.add_argument(
            ...
            default=None,
            required=False,
            help="input text and lengths with txt format (default: see configs/demo)",
        )
        group.add_argument(
            "--out_dir",
            type=str,
    ...
    OmegaConf.register_new_resolver("env", _resolve_env_with_default)
    ...
    if phase == "demo":
        ...
        if params.task:
            ...
            cfg.model.params.task = params.task
    ...
    if cfg.DEBUG:
        ...
        cfg.LOGGER.VAL_EVERY_STEPS = 250
