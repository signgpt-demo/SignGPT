...
def calculate_frechet_distance_np(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # Ensure that covariances do not contain NaNs or Infs
    ...
    # Adding a small epsilon to the diagonal to improve numerical stability
    sigma1 = sigma1 + np.eye(sigma1.shape[0]) * eps
    sigma2 = sigma2 + np.eye(sigma2.shape[0]) * eps

    # Product might be almost singular, calculate the square root of the covariance product
    covmean, _ = scipy.linalg.sqrtm(sigma1.dot(sigma2), disp=False)

    # If the square root is not finite, adjust the covariance estimates
