...
class BaseLosses(nn.Module):
    ...
    def _update_loss(self, loss: str, outputs, inputs, length=None):
        ...
        if 'recons_' in loss and loss != 'recons_hand_angle':
            val = self._losses_func[loss](outputs, inputs)
        else:
            val = self._losses_func[loss](outputs, inputs)
        weighted_loss = self._params[loss] * val
        getattr(self, loss).add_(weighted_loss.detach())
    ...
    def loss2logname(self, loss: str, split: str):
        ...
        if loss == "total":
            ...
            try:
                ...
                loss_type, name, sub_name = loss.split("_")
                log_name = f"{loss_type}/{name}/{sub_name}/{split}"
        return log_name
