...
#
# Quaternion utilities borrowed from HumanML3D and adapted for the
# 73-joint sign-language skeleton. Kept numerically identical to the
# original preprocessing code (note the float32 epsilon).
...
def qrot(q, v):
    """Rotate vector(s) v by the rotation described by quaternion(s) q.

    Expects tensors of shape (*, 4) and (*, 3) with matching leading dims.
...
def qeuler(q, order, epsilon=0, deg=True):
    """Convert quaternion(s) q to Euler angles in the given axis order."""
    assert q.shape[-1] == 4
    ...
    if order == 'xyz':
        ...
        x = torch.asin(torch.clamp(2 * (q0 * q1 - q2 * q3), -1 + epsilon, 1 - epsilon))
        y = torch.atan2(2 * (q1 * q3 + q0 * q2), 1 - 2 * (q1 * q1 + q3 * q3))
    ...
    return torch.stack((x, y, z), dim=1).view(original_shape) * (180 / np.pi if deg else 1)
...
def qfix(q):
    """Enforce quaternion continuity along the time dimension.

    Selects between q and -q per frame to minimize the distance to the
    previous frame. Expects a tensor of shape (L, J, 4).
    """
...
def euler2quat(e, order, deg=True):
    ...
    for coord in order:
        if coord == 'x':
            ...
            raise ValueError(f'Unknown axis: {coord}')
        if result is None:
            result = r
        else:
...
def expmap_to_quaternion(e):
    ...
    Exponential Map". Expects a tensor of shape (*, 3).
    """
    assert e.shape[-1] == 3
...
def euler_to_quaternion(e, order):
    """Convert Euler angles (numpy) to quaternions (numpy)."""
    assert e.shape[-1] == 3
...
def quaternion_to_matrix(quaternions):
    """Convert rotations given as quaternions (real part first) to matrices.
...
def qpow(q0, t, dtype=torch.float):
    """Power of a quaternion: q0 (tensor of quaternions), t (powers)."""
    q0 = qnormalize(q0)
    theta0 = torch.acos(q0[..., 0])

    # Avoid NaNs when theta0 is close to zero
...
def qslerp(q0, q1, t):
    """Spherical linear interpolation between q0 and q1 at points t."""
    q0 = qnormalize(q0)
    q1 = qnormalize(q1)
...
def qbetween(v0, v1):
    """Find the quaternion that rotates vector v0 onto vector v1."""
...
def matrix_to_quat(R):
    ...
    Reference: https://github.com/duolu/pyrotation
    """
